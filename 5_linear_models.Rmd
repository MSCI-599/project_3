...
lm_3_predict = lm_3 %>%
  broom::augment(penguins_lm_3, se_fit=TRUE) %>% # augment() instead of predict()
...
lm_3_predict = lm_3 %>%
  ...
  mutate(lwr = .fitted - 1.96 * .se.fit, 
         upr = .fitted + 1.96 * .se.fit) # Calculate 95% C.I. using SE
# Plot the data and the model predictions
ggplot() +
...
# flipper_length + species is a better fit than bill_length + species
```
...
lm_gentoo_3_predict = lm_gentoo_3 %>%
  augment(newdata = newdata, se_fit=TRUE) %>% # ?augment.lm
  mutate(lwr = .fitted - 1.96 * .se.fit, upr = .fitted + 1.96 * .se.fit) # Calculate 95% C.I. using SE
...
# Note: penguins with body_mass_g=NA or sex=NA can't be included in ANOVA model
penguins %>%
  filter(!is.na(sex),
         !is.na(body_mass_g)) %>% 
  group_by(species) %>%
